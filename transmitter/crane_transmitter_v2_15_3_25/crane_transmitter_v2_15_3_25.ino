...
uint32_t buttonColors[] = {
  0xFF0000,  // Red (Button 1)
  0x00FF00,  // Green (Button 2)
  0x0000FF,  // Blue (Button 3)
  0xFFFF00,  // Yellow (Button 4)
  0xFF00FF,  // Purple (Button 5)
  0x00FFFF   // Cyan (Button 6)
};
...
void sendButtonStates() {
  ...
  if (mqttClient.connected()) {
    ...
    if (mqttClient.connected()) {
      mqttClient.publish(MQTT_TOPIC, jsonBuffer);
      Serial.print("MQTT message sent after reconnect: ");
      Serial.println(jsonBuffer);
    }
  }
}
...
void printWakeupReason() {
  ...
  switch(wakeup_reason) {
    case ESP_SLEEP_WAKEUP_EXT1: {
      ...
      Serial.println("Wakeup caused by external signal using EXT1");
      
      // Determine which button woke up the device
      for (int i = 0; i < numButtons; i++) {
        if (wakeup_pin_mask & (1ULL << buttonPins[i])) {
          Serial.print("Wakeup button: GPIO ");
...
void setup() {
  ...
  Serial.println("ESP32-S3 WS2812 LED Control with Deep Sleep and MQTT");
...
void loop() {
  ...
  if (!anyButtonPressed && buttonPressed) {
    ...
    Serial.println("All buttons released, preparing for sleep");
  }
